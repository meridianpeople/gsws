...
import { getDomainContacts, setDomainContacts } from '@/lib/api/domains'
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ domain: string }> }) {
  ...
  try {
    const data = await getDomainContacts(auth.pkg.twentyi_package_id, auth.pkg.domain_id || domain)
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ domain: string }> }) {
  ...
  try {
    const data = await setDomainContacts(auth.pkg.twentyi_package_id, auth.pkg.domain_id || domain, body)
    return NextResponse.json(data)
  } catch (err: any) {
    return NextResponse.json({ error: err.message }, { status: 500 })
  }
