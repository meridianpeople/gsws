...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { featureId, feature, enabled } = await req.json()
    await client.post(`/package/${id}/web/addCdnFeature`, {
      features: [{ CdnFeatureId: featureId, Feature: feature, Enabled: String(enabled) }]
    })
