...
export default function EmailManager({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {loading && <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-secondary)', fontSize: '13px' }}>Loading…</div>}

      {/* MAILBOXES */}
      {!loading && tab === 'mailboxes' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>{mailboxes.length} account{mailboxes.length !== 1 ? 's' : ''} for @{selectedDomain}</p>
          ...
          {showAdd && (
            <div className="gsws-card" style={{ border: '2px solid #1a6ef5' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>@{selectedDomain}</span>
                  </div>
                </div>
                <div>
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ fontSize: '11px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Quota (MB) — 0 = unlimited</label>
                <input type="number" value={newMailbox.quotaMB} onChange={e => setNewMailbox(m => ({ ...m, quotaMB: Number(e.target.value) }))}
                  style={{ width: '150px', height: '34px', border: '1px solid var(--card-border-hover)', borderRadius: '6px', fontSize: '13px', padding: '0 10px', fontFamily: 'inherit' }} />
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={() => setShowAdd(false)} style={{ height: '32px', padding: '0 14px', background: 'var(--card-bg)', border: '1px solid var(--card-border-hover)', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
              </div>
            </div>
          )}
          <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: '10px', overflow: 'hidden' }}>
            {mailboxes.length === 0 ? (
              <div style={{ padding: '48px', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>No email accounts</p>
                <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '4px' }}>Create your first account for @{selectedDomain}</p>
              </div>
            ) : (
                ...
                <tbody>
                  {mailboxes.map((mb: any, i: number) => (
                    <tr key={i}>
                      ...
                      <td style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{mb.quotaMB ? `${mb.quotaMB} MB` : 'Unlimited'}</td>
                      <td style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{mb.usageMB ? `${Number(mb.usageMB).toFixed(1)} MB` : '—'}</td>
                      ...
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <div style={{ display: 'flex', gap: '4px' }}>
                            <button onClick={() => handleWebmail(mb.local)} style={{ padding: '0 10px', height: '24px', border: '1px solid #1a6ef5', borderRadius: '4px', fontSize: '11px', color: '#1a6ef5', background: 'var(--card-bg)', cursor: 'pointer', fontFamily: 'inherit' }}>Webmail ↗</button>
                            <button onClick={() => { setChangingPwd(changingPwd === mb.local ? null : mb.local); setNewEmailPwd('') }} style={{ padding: '0 10px', height: '24px', border: '1px solid #854f0b', borderRadius: '4px', fontSize: '11px', color: '#854f0b', background: 'var(--card-bg)', cursor: 'pointer', fontFamily: 'inherit' }}>Change password</button>
                            <button onClick={() => { if(confirm(`Delete ${mb.local}@${selectedDomain}?`)) apiDelete('mailbox', { local: mb.local }).then(() => { showSuccess('Deleted'); loadEmailData() }).catch(e => showError(e.message)) }} style={{ padding: '0 10px', height: '24px', border: '1px solid #f5c1c1', borderRadius: '4px', fontSize: '11px', color: '#a32d2d', background: 'var(--card-bg)', cursor: 'pointer', fontFamily: 'inherit' }}>Delete</button>
                          </div>
                          {changingPwd === mb.local && (
                            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                              <input
                                ...
                                style={{ flex: 1, height: '28px', border: '1px solid var(--card-border-hover)', borderRadius: '4px', fontSize: '12px', padding: '0 8px', fontFamily: 'inherit' }}
                              />
                              ...
                              <button onClick={() => { setChangingPwd(null); setNewEmailPwd('') }}
                                style={{ height: '28px', padding: '0 8px', background: 'var(--card-bg)', border: '1px solid var(--card-border-hover)', borderRadius: '4px', fontSize: '11px', cursor: 'pointer', fontFamily: 'inherit' }}>
                                Cancel
                              </button>
                            </div>
                          )}
      ...
      {!loading && tab === 'forwarders' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>{forwarders.length} forwarder{forwarders.length !== 1 ? 's' : ''}</p>
          ...
          {showAdd && (
            <div className="gsws-card" style={{ border: '2px solid #1a6ef5' }}>
              <h3 style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '14px' }}>Add forwarder</h3>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '8px', alignItems: 'end', marginBottom: '12px' }}>
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>From</label>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <div style={{ fontSize: '18px', color: 'var(--text-secondary)', paddingBottom: '4px' }}>→</div>
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>To</label>
          ...
          <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: '10px', overflow: 'hidden' }}>
            {forwarders.length === 0 ? (
              <div style={{ padding: '48px', textAlign: 'center' }}><p style={{ fontSize: '32px', marginBottom: '8px' }}>↪️</p><p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>No forwarders</p><p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '4px' }}>Forward emails from @{selectedDomain} to any address</p></div>
            ) : (
              <table className="gsws-table">
                ...
                <tbody>
                  {forwarders.map((f: any, i: number) => (
                    <tr key={i}>
                      ...
                      <td><button onClick={() => apiDelete('forwarder', { local: f.local }).then(() => { showSuccess('Deleted'); loadEmailData() }).catch(e => showError(e.message))} style={{ padding: '0 10px', height: '24px', border: '1px solid #f5c1c1', borderRadius: '4px', fontSize: '11px', color: '#a32d2d', background: 'var(--card-bg)', cursor: 'pointer', fontFamily: 'inherit' }}>Delete</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {!loading && tab === 'catchall' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div className="gsws-card">
            <h3 style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '4px' }}>Catch-All Forwarder</h3>
            <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '14px' }}>Forward any email sent to a non-existent address at @{selectedDomain} to a specific email address.</p>
            {catchAlls.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '14px' }}>
                {catchAlls.map((ca: any, i: number) => (
                  <div key={ca.id || i} style={{ padding: '10px 14px', background: '#eaf3de', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <p style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '2px' }}>anything@{selectedDomain} →</p>
                    ...
                    <button onClick={() => { apiDelete('catchall', { catchallId: ca.id }).then(() => { showSuccess('Catch-all removed'); loadEmailData() }).catch((e: any) => showError(e.message)) }}
                      style={{ padding: '0 10px', height: '26px', border: '1px solid #f5c1c1', borderRadius: '4px', fontSize: '11px', color: '#a32d2d', background: 'var(--card-bg)', cursor: 'pointer', fontFamily: 'inherit' }}>Remove</button>
                  </div>
                ))}
              </div>
            )}
            <div>
              <label style={{ fontSize: '11px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Destination email address</label>
          ...
          {/* Wildcard forwarders */}
          <div className="gsws-card">
            <h3 style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '4px' }}>Wildcard Forwarder</h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '14px' }}>
                {wildcards.map((w: any, i: number) => (
                  <div key={w.id || i} style={{ padding: '10px 14px', background: '#e8f0fe', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <p style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '2px' }}>*@{selectedDomain} →</p>
            ...
            <div>
              <label style={{ fontSize: '11px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Destination domain</label>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <span style={{ fontSize: '13px', color: 'var(--text-secondary)', flexShrink: 0 }}>*@{selectedDomain} →</span>
                <span style={{ fontSize: '13px', color: 'var(--text-secondary)', flexShrink: 0 }}>*@</span>
      ...
      {!loading && tab === 'autoresponders' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>{autoresponders.length} autoresponder{autoresponders.length !== 1 ? 's' : ''}</p>
          ...
          {showAdd && (
            <div className="gsws-card" style={{ border: '2px solid #1a6ef5' }}>
              <h3 style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '14px' }}>Add autoresponder</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '12px' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <input value={newAutoresponder.local} onChange={e => setNewAutoresponder(a => ({ ...a, local: e.target.value }))} placeholder="info"
                      style={{ width: '180px', height: '34px', border: '1px solid var(--card-border-hover)', borderRadius: '6px', fontSize: '13px', fontFamily: 'ui-monospace, monospace', padding: '0 10px' }} />
                    <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>@{selectedDomain}</span>
                  </div>
                </div>
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Subject</label>
                ...
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Message</label>
                  <textarea value={newAutoresponder.message} onChange={e => setNewAutoresponder(a => ({ ...a, message: e.target.value }))} placeholder="Thank you for contacting us..."
                    rows={4} style={{ width: '100%', border: '1px solid var(--card-border-hover)', borderRadius: '6px', fontSize: '13px', padding: '8px 10px', boxSizing: 'border-box', fontFamily: 'inherit', resize: 'vertical' }} />
                </div>
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: '10px', overflow: 'hidden' }}>
            {autoresponders.length === 0 ? (
              <div style={{ padding: '48px', textAlign: 'center' }}><p style={{ fontSize: '32px', marginBottom: '8px' }}>🤖</p><p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>No autoresponders</p><p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '4px' }}>Send automatic replies to incoming emails</p></div>
            ) : (
              <table className="gsws-table">
                ...
                <tbody>
                  {autoresponders.map((a: any, i: number) => (
                    <tr key={i}>
                      ...
                      <td><button style={{ padding: '0 10px', height: '24px', border: '1px solid #f5c1c1', borderRadius: '4px', fontSize: '11px', color: '#a32d2d', background: 'var(--card-bg)', cursor: 'pointer', fontFamily: 'inherit' }}>Delete</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {!loading && tab === 'dkim' && (
        <div className="gsws-card">
          <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '8px' }}>🔑 DKIM (DomainKeys)</h3>
          <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '16px' }}>DKIM adds a digital signature to outgoing emails, improving deliverability and preventing spoofing.</p>
          {dkim.length > 0 ? (
            <div>
      ...
      {!loading && tab === 'dmarc' && (
        <div className="gsws-card">
          <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '8px' }}>🛡️ DMARC Policy</h3>
          <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '16px' }}>DMARC tells receiving mail servers what to do with emails that fail SPF or DKIM checks.</p>
          {dmarc.length > 0 ? (
      ...
      {!loading && tab === 'spam' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div className="gsws-card">
            <h3 style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '8px' }}>Block email addresses</h3>
            <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '12px' }}>Emails from blocked addresses will be rejected automatically.</p>
            ...
            {spamBlacklist.length === 0 ? (
              <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>No blocked addresses.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {spamBlacklist.map((s: any, i: number) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 12px', background: '#fcebeb', borderRadius: '6px' }}>
                    ...
                    <button style={{ padding: '0 8px', height: '22px', border: '1px solid #f5c1c1', borderRadius: '4px', fontSize: '11px', color: '#a32d2d', background: 'var(--card-bg)', cursor: 'pointer', fontFamily: 'inherit' }}>Remove</button>
                  </div>
                ))}
              </div>
            )}
      ...
      {!loading && tab === 'settings' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="gsws-card">
            <h3 style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '14px' }}>Mail server settings</h3>
            {[
              ...
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--card-border)', fontSize: '12px' }}>
                <span style={{ color: 'var(--text-secondary)', width: '140px' }}>{label}</span>
                <span style={{ fontFamily: 'ui-monospace, monospace', fontWeight: 500, color: 'var(--text-primary)' }}>{value}</span>
                <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>{note}</span>
              </div>
            ))}
          </div>
        </div>
