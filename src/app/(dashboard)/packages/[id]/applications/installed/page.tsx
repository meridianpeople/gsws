...
function InstalledContent({ packageId }: { packageId: string }) {
  ...
  return (
    <div style={{ maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {/* Success header */}
      <div style={{ textAlign: 'center', padding: '32px 20px', background: '#fff', borderRadius: '12px', border: '1px solid #ebebeb' }}>
        <div style={{ fontSize: '48px', marginBottom: '12px' }}>{APP_ICONS[app] || '📦'}</div>
        <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#0a0a0a', marginBottom: '6px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          {tempSiteUrl && (
            <div style={{ padding: '10px 14px', background: '#1a2840', borderRadius: '8px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                <a href={tempSiteUrl} target="_blank" style={{ fontFamily: 'ui-monospace, monospace', fontSize: '12px', color: '#9a9a9a', textDecoration: 'none', flex: 1 }}>{tempSiteUrl}</a>
                <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
                  ...
                  <a href={tempSiteUrl} target="_blank" style={{ height: '24px', padding: '0 8px', background: '#2a3a50', color: '#9a9a9a', border: 'none', borderRadius: '4px', fontSize: '11px', cursor: 'pointer', fontFamily: 'inherit', textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>Open ↗</a>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="gsws-card">
        <h3 style={{ fontSize: '13px', fontWeight: 600, color: '#0a0a0a', marginBottom: '12px' }}>Next steps</h3>
        {[
          ...
          <div key={s.step} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '8px 0', borderBottom: '1px solid #ebebeb', fontSize: '13px', color: '#5a5a5a' }}>
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        ...
        <Link href={`/packages/${packageId}/applications`}
          style={{ height: '42px', padding: '0 20px', display: 'flex', alignItems: 'center', background: '#fff', color: '#0a0a0a', borderRadius: '8px', fontSize: '13px', fontWeight: 500, textDecoration: 'none', border: '1px solid #d4d4d4' }}>
          ← Back to apps
        </Link>
      </div>
...
export default function InstalledPage({ params }: { params: { id: string } }) {
  return (
    <Suspense fallback={<div style={{ padding: '48px', textAlign: 'center', color: '#9a9a9a' }}>Loading…</div>}>
