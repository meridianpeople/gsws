...
export default function WordPressManager({ packageId, domainName, wpVersion, wpSettings, plugins, themes, admins, staging, updatesAvailable, wizardRequired }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div className="gsws-card">
              <h3 style={{ fontSize: '13px', fontWeight: 600, color: '#0a0a0a', marginBottom: '14px' }}>Site information</h3>
              {[
                ...
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #ebebeb', fontSize: '12px' }}>
                  ...
                  <span style={{ fontWeight: 500, color: '#0a0a0a', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</span>
                </div>
              ))}
            </div>
            ...
            {/* Quick actions */}
            <div className="gsws-card">
              <h3 style={{ fontSize: '13px', fontWeight: 600, color: '#0a0a0a', marginBottom: '14px' }}>Quick actions</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {[
                  ...
                  <button key={a.label} onClick={a.action}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: '#f7f7f7', border: '1px solid #ebebeb', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
                    <div>
                      <p style={{ fontSize: '12px', fontWeight: 600, color: '#0a0a0a' }}>{a.label}</p>
                      <p style={{ fontSize: '11px', color: '#9a9a9a', marginTop: '2px' }}>{a.desc}</p>
                    </div>
      ...
      {tab === 'plugins' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ fontSize: '13px', color: '#9a9a9a' }}>{pluginList.length} plugins · {activePlugins} active · {pluginUpdates} updates</p>
            {pluginUpdates > 0 && (
              ...
              <div key={plugin.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', border: `1px solid ${plugin.update === 'available' ? '#f0c070' : '#ebebeb'}`, borderRadius: '8px', background: plugin.update === 'available' ? '#fffbf0' : '#fff' }}>
                <div style={{ flex: 1, marginRight: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 700, color: '#0a0a0a' }}>{plugin.title}</span>
                  ...
                  <p style={{ fontSize: '11px', color: '#9a9a9a', marginTop: '2px' }}>v{plugin.version}</p>
                </div>
      ...
      {tab === 'themes' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <p style={{ fontSize: '13px', color: '#9a9a9a' }}>{themeList.length} themes installed</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
            {themeList.map(theme => (
              <div key={theme.name} style={{ padding: '14px', background: '#fff', border: `2px solid ${theme.status === 'active' ? '#1a6ef5' : theme.update === 'available' ? '#f0c070' : '#ebebeb'}`, borderRadius: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  <span style={{ fontSize: '20px' }}>🎨</span>
                  <div>
                    <p style={{ fontSize: '12px', fontWeight: 700, color: '#0a0a0a' }}>{theme.title}</p>
                    <p style={{ fontSize: '10px', color: '#9a9a9a' }}>v{theme.version}</p>
                  </div>
                </div>
      ...
      {tab === 'users' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ fontSize: '13px', color: '#9a9a9a' }}>{admins.length} WordPress admin{admins.length !== 1 ? 's' : ''}</p>
          </div>
          <div className="gsws-card" style={{ padding: 0, overflow: 'hidden' }}>
            {admins.length === 0 ? (
              <div style={{ padding: '32px', textAlign: 'center', color: '#9a9a9a', fontSize: '13px' }}>No admin users found.</div>
            ) : (
                ...
                <tbody>
                  {admins.map((u: any) => (
                    <tr key={u.ID}>
                      ...
                      <td style={{ fontFamily: 'ui-monospace, monospace', fontSize: '11px', color: '#9a9a9a' }}>{u.user_email}</td>
                      ...
                      <td style={{ fontSize: '11px', color: '#9a9a9a' }}>{new Date(u.user_registered).toLocaleDateString('en-GB')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {tab === 'settings' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="gsws-card">
            <h3 style={{ fontSize: '13px', fontWeight: 600, color: '#0a0a0a', marginBottom: '14px' }}>WordPress site settings</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
              {[
                ...
                <div key={field.key}>
                  <label style={{ fontSize: '11px', color: '#9a9a9a', display: 'block', marginBottom: '4px' }}>{field.label}</label>
      ...
      {tab === 'staging' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="gsws-card">
            <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#0a0a0a', marginBottom: '8px' }}>🧪 Staging environment</h3>
            ...
            {staging.length > 0 ? (
                ...
                {staging.map((s: any, i: number) => (
                  <div key={i} style={{ padding: '14px 16px', background: '#eaf3de', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      ...
                      <p style={{ fontSize: '11px', color: '#9a9a9a', marginTop: '2px' }}>{s.url || s.domain || 'Staging URL available'}</p>
                    </div>
                    <div style={{ display: 'flex', gap: '6px' }}>
              ...
              <div>
                <div style={{ padding: '16px', background: '#f7f7f7', borderRadius: '8px', marginBottom: '14px', fontSize: '12px', color: '#9a9a9a' }}>
      ...
      {tab === 'search' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="gsws-card">
            <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#0a0a0a', marginBottom: '8px' }}>🔍 Search & Replace</h3>
            <p style={{ fontSize: '12px', color: '#9a9a9a', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '14px' }}>
              <div>
                <label style={{ fontSize: '11px', color: '#9a9a9a', display: 'block', marginBottom: '4px' }}>Search for</label>
              ...
              <div>
                <label style={{ fontSize: '11px', color: '#9a9a9a', display: 'block', marginBottom: '4px' }}>Replace with</label>
