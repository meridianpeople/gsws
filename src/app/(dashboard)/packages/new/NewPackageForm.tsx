    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '900px' }}>
      <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h1 style={{ fontSize: '20px', fontWeight: 600, color: 'var(--text-primary)' }}>Create a hosting package</h1>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '3px' }}>Select a domain and hosting plan. First month charged from credit.</p>
          </div>
          {balance !== null && (
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={() => setWarning(null)}
                  style={{ height: '34px', padding: '0 16px', background: 'var(--card-bg)', color: '#854f0b', border: '1px solid #f0c070', borderRadius: '6px', fontSize: '12.5px', cursor: 'pointer', fontFamily: 'inherit' }}>
                  Cancel
                </button>
              </div>
            </div>
      ...
      {loading ? (
        <div className="gsws-card" style={{ textAlign: 'center', padding: '48px', color: 'var(--text-secondary)', fontSize: '13px' }}>Loading…</div>
      ) : (
        <>
          {/* Step 1 — Domain */}
              ...
              {types.map(pkg => {
                ...
                return (
                  <div key={pkg.id} onClick={() => setSelectedType(pkg.id)}
                    ...
                    <p style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '4px' }}>{pkg.label}</p>
                    <p style={{ fontSize: '11px', color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: '12px' }}>{pkg.description}</p>
                    <div style={{ borderTop: '1px solid var(--card-border)', paddingTop: '10px' }}>
                      <p style={{ fontSize: '18px', fontWeight: 800, color: info.color }}>£{pkg.monthlyExVat.toFixed(2)}<span style={{ fontSize: '11px', fontWeight: 400, color: 'var(--text-secondary)' }}>/mo ex VAT</span></p>
                      <p style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '2px' }}>£{pkg.monthlyIncVat.toFixed(2)}/mo inc VAT</p>
                    </div>
                  </div>
                )
              })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px', marginBottom: '14px' }}>
                {features.map((f, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', background: 'var(--card-bg-elevated)', borderRadius: '6px' }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--text-primary)' }}>{f}</span>
                  </div>
                ))}
              </div>
