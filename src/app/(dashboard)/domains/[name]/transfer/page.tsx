...
export default async function TransferPage({ params }: { params: Promise<{ name: string }> }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '700px' }}>
      <div>
        <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
          ...
          <Link href={"/domains/" + name} style={{ color: '#1a6ef5' }}>{domainName}</Link> › Transfer
        </div>
        <h1 style={{ fontSize: '20px', fontWeight: 600, color: 'var(--text-primary)' }}>Transfer</h1>
      </div>
      <div className="gsws-card" style={{ textAlign: 'center', padding: '48px', border: '1px dashed var(--card-border-hover)' }}>
        <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Transfer management coming soon.</p>
      </div>
    </div>
  )
}
